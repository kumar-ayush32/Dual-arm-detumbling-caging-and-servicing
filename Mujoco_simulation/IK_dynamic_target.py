...
# Trajectory curvature control

# V_CRUISE    : nominal cruise speed (m/s).
# T = dist / V_CRUISE is the baseline duration.

# ALPHA_CURVE : max lateral deviation as a fraction of straight-line dist.
# 0.05 = very straight lines, 0.20 = visibly curved arcs.

# DAMP_RADIUS : within this many metres of the target, start_vel is tapered
# to zero so the arm doesn't arc widely when nearly done.

# T_MIN / T_MAX : hard clamps on the computed duration (seconds).
...
def read_actual_q(data, qpos_adr):
    """
    Read the actual joint angles for the 3-DOF chain from data.qpos.
    Returns (3,) array [q_hindarm, q_forearm, q_hand] in MuJoCo radians.
    This is the ONLY correct way to get the arm's true current state at startup
    or after any external disturbance — never assume zeros.
    """
    return np.array([
...
def ee_world_velocity(model, data, ee_id, qpos_adr):
    """
    Compute EE world-frame velocity [Vx, Vz] using the position Jacobian.

    data.cvel is expressed in the LOCAL body frame — using it directly as
    world-frame velocity is wrong and causes the quintic initial-velocity
    condition to be incorrect, leading to arm overshoot.

    The Jacobian approach (mj_jacBody) always returns world-frame columns,
    so J_pos @ qvel gives the correct world-frame EE velocity.
    """
    nv = model.nv
...
def solve_ik_trig(target_xz, phi_world, elbow_up=True):
    ...
    # Wrist position (subtract L3 tool along hand direction)
    ...
    q1    = alpha - beta
    q3    = phi_geom - q1 - q2     # closure: q1 + q2 + q3 = phi_geom
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            print(
                ...
                f"phi={np.degrees(phi_now):.1f}°  "
                f"q=[{q_deg[0]:6.2f} {q_deg[1]:6.2f} {q_deg[2]:6.2f}]°  "
